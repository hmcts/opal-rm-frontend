---
name: opal-rm-frontend-review-guidelines
description: Automated review rules for opal-rm-frontend PRs. Use when asked to review code, run @codex review, or provide PR feedback; apply severity levels and the required comment format.
---

# OPAL RM Frontend Review Guidelines

## Overview

Apply these rules when reviewing changes in `opal-rm-frontend`; focus on P0/P1 blockers and use the required comment format.
...
- Assume an Angular v20+ standalone SSR starter using the GOV.UK/HMCTS design system.
- Preserve the starter repo boundary: dashboard-level routing, shared auth/bootstrap, and a deliberately reduced app surface.
- Prefer modern Angular primitives: standalone components, template control flow, signals, and direct imports.
- Ensure accessibility aligns with WCAG 2.2 AA.
- Avoid changes that silently re-couple the starter to full `opal-frontend` flows unless the task explicitly requires that expansion.
...
- Do not break the SSO-disabled login path, dashboard landing route, or the minimal Cypress/Vitest contract without replacing them with equivalent starter-safe behavior.
...
- Keep dashboard starter routes coherent; avoid leaving inactive route trees or half-copied wrappers behind.
- Prefer copying only the shared helpers, docs, and tests the starter actually uses.
- When pulling from `opal-frontend`, trim full-app assumptions rather than preserving them by default.
...
- Prefer Angular Testing Library/Harnesses when present; avoid brittle DOM selectors when a better stable seam exists.
- Keep Cypress additions aligned with the starter surface: sign-in/sign-out, dashboard landing, primary nav, and shell behaviour unless the task intentionally expands coverage.
...
- Prefer lazy-loaded routes and deferrable views.
