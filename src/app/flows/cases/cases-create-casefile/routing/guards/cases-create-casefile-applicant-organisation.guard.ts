...
export const casesCreateCasefileApplicantOrganisationGuard = hasFlowStateGuard(
  () => {
    const store = inject(CasesCreateCasefileStore);
    return { caseTypeSelection: store.caseTypeSelection() };
  },
  (state) => isCasesCreateCasefileOrganisationApplicantSelection(state.caseTypeSelection),
