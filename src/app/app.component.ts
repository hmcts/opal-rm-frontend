...
import { DASHBOARD_ROUTING_PATHS } from './pages/dashboard/constants/dashboard-routing-paths.constant';
import { getAccessiblePrimaryNavigationItems } from './pages/dashboard/utils/dashboard-section-permissions.utils';
...
export class AppComponent implements OnInit, OnDestroy {
  ...
  /**
   * Reads the dashboard type from `/dashboard/:dashboardType`.
   * Only falls back to the default tab when `/dashboard` has no type segment.
  ...
  private getDashboardTypeFromUrl(url: string): DashboardPageType | '' {
    ...
    const dashboardRootIndex = segments.indexOf(DASHBOARD_ROUTING_PATHS.root);
    const isDashboardRoute = dashboardRootIndex === 0;

    if (!isDashboardRoute) {
      return '';
    }

    const dashboardType = segments[dashboardRootIndex + 1];
