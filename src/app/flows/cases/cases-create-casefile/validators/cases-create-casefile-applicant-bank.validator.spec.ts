...
} from './cases-create-casefile-applicant-bank.validator';
import { casesCreateCasefileApplicantIndividualTrimRequiredValidator } from '../cases-create-casefile-applicant-individual/validators/cases-create-casefile-applicant-individual-trim-required.validator';

describe('CasesCreateCasefileApplicantBankValidator', () => {
  describe('casesCreateCasefileApplicantUkSortCodeValidator', () => {
    it.each(['112233', '11-22-33'])('accepts the valid UK sort code %s', (value) => {
      expect(casesCreateCasefileApplicantUkSortCodeValidator(new FormControl(value))).toBeNull();
    });

    it('returns ukSortCodePattern when the sort code contains letters', () => {
      expect(casesCreateCasefileApplicantUkSortCodeValidator(new FormControl('11AB33'))).toEqual({
        ukSortCodePattern: true,
      });
    });

    it('returns ukSortCodePattern when an otherwise valid sort code is wrapped in whitespace', () => {
      expect(casesCreateCasefileApplicantUkSortCodeValidator(new FormControl(' 11-22-33 '))).toEqual({
        ukSortCodePattern: true,
      });
    });

    it.each(['11223', '1122334'])('returns ukSortCodeLength when %s has the wrong number of digits', (value) => {
      expect(casesCreateCasefileApplicantUkSortCodeValidator(new FormControl(value))).toEqual({
  ...
  describe('casesCreateCasefileApplicantUkAccountNumberValidator', () => {
    it.each(['123456', '1234567', '12345678'])('accepts the valid UK account number %s', (value) => {
      expect(casesCreateCasefileApplicantUkAccountNumberValidator(new FormControl(value))).toBeNull();
    });

    it('returns ukAccountNumberPattern when the account number contains letters', () => {
      expect(casesCreateCasefileApplicantUkAccountNumberValidator(new FormControl('12345A'))).toEqual({
    ...
    it('returns ukAccountNumberPattern when an otherwise valid account number is wrapped in whitespace', () => {
      expect(casesCreateCasefileApplicantUkAccountNumberValidator(new FormControl(' 123456 '))).toEqual({
        ukAccountNumberPattern: true,
      });
    });

    it.each(['12345', '123456789'])('returns ukAccountNumberLength when %s is outside 6 to 8 digits', (value) => {
      expect(casesCreateCasefileApplicantUkAccountNumberValidator(new FormControl(value))).toEqual({
        ukAccountNumberLength: true,
      });
    });
  });

  describe('casesCreateCasefileApplicantBicSwiftValidator', () => {
    it.each(['', '   ', 'DEUTDEFF', 'DEUTDEFF500', 'deutdeff'])(
      'accepts the optional valid BIC or SWIFT value %j',
      (value) => {
        expect(casesCreateCasefileApplicantBicSwiftValidator(new FormControl(value))).toBeNull();
      },
    );
    ...
    it.each(['DEUTDEFF5', 'DEUTDEFF50', 'DE1TDEFF', 'DEUT1EFF', 'DEUTDE-F'])(
      ...
      (value) => {
        expect(casesCreateCasefileApplicantBicSwiftValidator(new FormControl(value))).toEqual({
    ...
    it('returns internationalIdentifierPattern when an otherwise valid BIC or SWIFT value is wrapped in whitespace', () => {
      expect(casesCreateCasefileApplicantBicSwiftValidator(new FormControl(' ABCD1234 '))).toEqual({
        internationalIdentifierPattern: true,
      });
    });
  });

  describe('casesCreateCasefileApplicantIbanValidator', () => {
    it.each(['', '   ', 'GB82WEST12345698765432', 'DE89370400440532013000', 'gb82west12345698765432'])(
      'accepts the optional valid IBAN value %j',
      (value) => {
        expect(casesCreateCasefileApplicantIbanValidator(new FormControl(value))).toBeNull();
      },
    );
    ...
    it.each([
      ...
      expect(casesCreateCasefileApplicantIbanValidator(new FormControl(value))).toEqual({
        internationalIdentifierPattern: true,
      });
    });

    it('returns internationalIdentifierPattern when an otherwise valid IBAN value is wrapped in whitespace', () => {
      expect(casesCreateCasefileApplicantIbanValidator(new FormControl(' GB82WEST12345698765432 '))).toEqual({
        internationalIdentifierPattern: true,
      });
    });
  });

  describe('casesCreateCasefileApplicantBranchSortCodeValidator', () => {
    it.each(['', '   ', '1', '123456789012'])('accepts the optional valid branch or sort code %j', (value) => {
      expect(casesCreateCasefileApplicantBranchSortCodeValidator(new FormControl(value))).toBeNull();
    });

    it('returns branchSortCodePattern when the branch or sort code contains letters', () => {
      expect(casesCreateCasefileApplicantBranchSortCodeValidator(new FormControl('123A'))).toEqual({
    ...
    it('returns branchSortCodePattern when an otherwise valid branch or sort code is wrapped in whitespace', () => {
      expect(casesCreateCasefileApplicantBranchSortCodeValidator(new FormControl(' 123456 '))).toEqual({
        branchSortCodePattern: true,
      });
    });

    it('returns branchSortCodeLength when the branch or sort code exceeds 12 digits', () => {
      expect(casesCreateCasefileApplicantBranchSortCodeValidator(new FormControl('1234567890123'))).toEqual({
        branchSortCodeLength: true,
      });
    });
  });

  describe('casesCreateCasefileApplicantInternationalIdentifierRequiredValidator', () => {
    it('accepts a populated BIC or SWIFT control', () => {
      const ibanControl = new FormControl('');
      const validator = casesCreateCasefileApplicantInternationalIdentifierRequiredValidator(ibanControl);
